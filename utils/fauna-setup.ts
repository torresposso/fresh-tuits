...
const secret = Deno.env.get("FAUNA_ADMIN_SECRET");

console.log("domain", domain, "secret", secret);
...
await faunaClient.query(q.CreateCollection({ name: "Post" }));
await faunaClient.query(q.CreateCollection({ name: "Comment" }));
